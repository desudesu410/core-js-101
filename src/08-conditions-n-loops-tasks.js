...
function isInsideCircle(circle, point) {
  ...
  const distance = ((pX - cX) ** 2 + (pY - cY) ** 2) ** 0.5;
  return distance < r;
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < digits.length; i += 1) {
    let cardNum = parseFloat(digits[i]);
...
function isBracketsBalanced(str) {
  ...
  while (bracketsInStr) {
    ...
    for (let i = 0; i < bracketsConfig.length; i += 1) {
      if (temp.includes(bracketsConfig[i])) {
        temp = temp.replace(bracketsConfig[i], '');
        bracketsInStr = true;
      }
    }
  }
...
function getMatrixProduct(m1, m2) {
  ...
  const res = [];
  if (colsM1 !== rowsM2) throw new Error('invalid sizes');
